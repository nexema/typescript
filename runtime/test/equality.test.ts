...
describe("Test equality methods on different types", () => {
  ...
  test("Test primitive equals", () => {
    expect(primitiveEquals(27, 27)).toBeTruthy();
    expect(primitiveEquals("hola", "hola")).toBeTruthy();
    expect(primitiveEquals(-9n, -9n)).toBeTruthy();
    expect(primitiveEquals(8n, 8)).toBeTruthy();
    expect(primitiveEquals(8n, "8")).toBeTruthy();
    expect(primitiveEquals(8, "8")).toBeTruthy();
    expect(primitiveEquals(true, true)).toBeTruthy();
    expect(primitiveEquals(null, 5)).toBeFalsy();
  });

  test("Test binary equals", () => {
    expect(
      binaryEquals(
        new Uint8Array([27, 42, 9, 124]),
        new Uint8Array([27, 42, 9, 124])
      )
    ).toBeTruthy();
    expect(
      binaryEquals(new Uint8Array([27, 42]), new Uint8Array([27]))
    ).toBeFalsy();
    expect(
      binaryEquals(new Uint8Array([27, 42]), new Uint8Array([42, 27]))
    ).toBeFalsy();
  });

  test("Test list equals", () => {
    expect(listEquals("int64", [8n, 7n, 12n], [8n, 7n, 12n])).toBeTruthy();
    expect(listEquals("string", ["a", "b", "c"], ["b", "c"])).toBeFalsy();
    expect(listEquals("string", ["a", "b", "c"], ["b", "c", "a"])).toBeFalsy();
    expect(
      listEquals("enum", [EnumA.blue, EnumA.red], [EnumA.blue, EnumA.red])
    ).toBeTruthy();
    expect(
      listEquals("enum", [EnumA.blue, EnumA.red], [EnumA.unknown, EnumA.red])
    ).toBeFalsy();
  });
